...
for dataset_name, data in datasets.items():
    ...
    processed_combinations = 0  # Track how many combinations have been processed
    ...
    for local_name, local_model in local_methods.items():
        for global_name, global_model in global_methods.items():
            for w_local, w_global in weight_pairs:
                ...
                processed_combinations += 1  # Increment by 1

                # Print every 10 combinations
                if processed_combinations % 10 == 0:
                    print(f"Processed {processed_combinations} method combinations so far in {dataset_name}")

    print(f" Finished processing dataset: {dataset_name} (Total Method Combinations Processed: {processed_combinations})\n")
...
# Save results to CSV
df_results.to_csv("adbench_results_17_InternetAds.csv", index=False)

print("Results saved to 'adbench_results_17_InternetAds.csv'.")
...
#files.download("adbench_results_17_InternetAds.csv")
