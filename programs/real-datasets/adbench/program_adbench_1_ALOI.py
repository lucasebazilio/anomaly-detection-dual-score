    ...
    for local_name, local_model in local_methods.items():
        for global_name, global_model in global_methods.items():
            for w_local, w_global in weight_pairs:
                ...
                # Print frequently for first 50 instances, then every 5
                if processed_instances <= 50 or processed_instances % 5 == 0:
...
# Save results to CSV
df_results.to_csv("adbench_results_1_ALOI.csv", index=False)

print("Results saved to 'adbench_results_1_ALOI.csv'.")
