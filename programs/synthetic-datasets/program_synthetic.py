...
# Store results
results = []
dataset_count = 0  # Initialize dataset counter

# Iterate through all synthetic datasets
for dataset_name, df in datasets.items():
    dataset_count += 1  # Increment counter

    # Print progress every 10 datasets
    if dataset_count % 10 == 0:
        print(f"Processed {dataset_count} datasets so far...")
...
df_synthetic_results = pd.DataFrame(results)

# Save synthetic results to CSV
df_synthetic_results.to_csv("synthetic_ad_results.csv", index=False)

print("Synthetic dataset anomaly detection results saved.")


# Save results to a CSV file
df_results.to_csv("synthetic_ad_results.csv", index=False)

print("Results saved to 'synthetic_ad_results.csv'")
