...
half_split = ceil(len(dataset_names) / 2)  # Split into first and second halves

# Select second half of datasets
datasets_second_half = {k: datasets[k] for k in dataset_names[half_split:]}

# Further split the second half into two smaller halves
second_half_dataset_names = list(datasets_second_half.keys())
quarter_split = ceil(len(second_half_dataset_names) / 2)
...
# Select datasets for Part 2.2 (remaining half of the second half)
datasets_part2_2 = {k: datasets_second_half[k] for k in second_half_dataset_names[quarter_split:]}

# Determine the split points for three equal parts
part_size = ceil(len(datasets_part2_2) / 3)
datasets_part2_2_1 = {k: datasets_part2_2[k] for k in list(datasets_part2_2.keys())[:part_size]}

print(f"Processing Part 2.2 (First Third): {len(datasets_part2_2_1)} datasets...")

# Store results
results = []
processed_count = 0  # Counter for processed datasets

# Iterate through datasets in the first third
for dataset_name, df in datasets_part2_2_1.items():
    ...
    # Print progress every 5 datasets
    if processed_count % 5 == 0:
        print(f"Processed {processed_count} datasets so far...")

# Convert to DataFrame
df_synthetic_part2_2_1 = pd.DataFrame(results)

# Save first third to CSV
df_synthetic_part2_2_1.to_csv("synthetic_ad_results_part2_2_1.csv", index=False)

print("Part 2.2 (First Third) of synthetic datasets processed and saved.")
...
# Select datasets for the second third of Part 2.2
datasets_part2_2_2 = {k: datasets_part2_2[k] for k in list(datasets_part2_2.keys())[part_size:2*part_size]}

print(f"Processing Part 2.2 (Second Third): {len(datasets_part2_2_2)} datasets...")

# Store results
results = []
processed_count = 0  # Counter for processed datasets

# Iterate through datasets in the second third
for dataset_name, df in datasets_part2_2_2.items():
    processed_count += 1  # Increment counter
...
# Convert to DataFrame
df_synthetic_part2_2_2 = pd.DataFrame(results)

# Save second third to CSV
df_synthetic_part2_2_2.to_csv("synthetic_ad_results_part2_2_2.csv", index=False)

print("Part 2.2 (Second Third) of synthetic datasets processed and saved.")



##########################

# Select datasets for the last third of Part 2.2
datasets_part2_2_3 = {k: datasets_part2_2[k] for k in list(datasets_part2_2.keys())[2*part_size:]}

print(f"Processing Part 2.2 (Last Third): {len(datasets_part2_2_3)} datasets...")

# Store results
results = []
processed_count = 0  # Counter for processed datasets

# Iterate through datasets in the last third
for dataset_name, df in datasets_part2_2_3.items():
    processed_count += 1  # Increment counter
...
# Convert to DataFrame
df_synthetic_part2_2_3 = pd.DataFrame(results)

# Save last third to CSV
df_synthetic_part2_2_3.to_csv("synthetic_ad_results_part2_2_3.csv", index=False)

print("Part 2.2 (Last Third) of synthetic datasets processed and saved.")
